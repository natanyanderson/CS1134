...
class CompactString:
    def __init__(self, orig_str):
        ...
        count = 1
        for char in range(1, len(orig_str)):
            if orig_str[char] == orig_str[char - 1]:
                count += 1
            else:
                self.data.add_last((orig_str[char - 1], count))
                count = 1
        if len(orig_str) != 0:
            self.data.add_last((orig_str[len(orig_str) - 1], count))

    def __add__(self, other):
        new_compstring = CompactString("")
        cursor = self.data.header.next
        while cursor.next.data is not None:
            new_compstring.data.add_last(cursor.data)
            cursor = cursor.next
        if cursor.data[0] == other.data.header.next.data[0]:
            new_compstring.data.add_last(
